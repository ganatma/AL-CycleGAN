...
def save_encoding(encodings, image_path, encoding_paths):
    short_path = ntpath.basename(image_path[0])
    name = os.path.splitext(short_path)[0]
    for label, im_encoding in encodings.items():
        image_name = '%s_%s.npy' % (name, label)
        save_path = os.path.join(encoding_paths, image_name)
        np.save(save_path, im_encoding.cpu().numpy())
